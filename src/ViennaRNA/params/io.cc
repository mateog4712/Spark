...
static char *last_param_file = NULL;

static int stack_dim[2] = {NBPAIRS + 1, NBPAIRS + 1};
static int stack_shift[2] = {1, 1};
static int mismatch_dim[3] = {NBPAIRS + 1, 5, 5};
static int mismatch_shift[3] = {1, 0, 0};
static int int11_dim[4] = {NBPAIRS + 1, NBPAIRS + 1, 5, 5};
static int int11_shift[4] = {1, 1, 0, 0};
static int int21_dim[5] = {NBPAIRS + 1, NBPAIRS + 1, 5, 5, 5};
static int int21_shift[5] = {1, 1, 0, 0, 0};
static int int22_dim[6] = {NBPAIRS + 1, NBPAIRS + 1, 5, 5, 5, 5};
static int int22_shift[6] = {1, 1, 1, 1, 1, 1};
static int int22_post[6] = {1, 1, 0, 0, 0, 0};
static int dangle_dim[2] = {NBPAIRS + 1, 5};
static int dangle_shift[2] = {1, 0};
...
static char *get_array1(char **content, size_t *line_no, int *arr, int size);

static void ignore_comment(char *line);
...
**/
static void rd_1dim(char **content, size_t *line_no, int *array, int dim, int shift);

static void rd_1dim_slice(char **content, size_t *line_no, int *array, int dim, int shift, int post);

static void rd_2dim(char **content, size_t *line_no, int *array, int dim[2], int shift[2]);

static void rd_2dim_slice(char **content, size_t *line_no, int *array, int dim[2], int shift[2], int post[2]);

static void rd_3dim(char **content, size_t *line_no, int *array, int dim[3], int shift[3]);

static void rd_3dim_slice(char **content, size_t *line_no, int *array, int dim[3], int shift[3], int post[3]);

static void rd_4dim(char **content, size_t *line_no, int *array, int dim[4], int shift[4]);

static void rd_4dim_slice(char **content, size_t *line_no, int *array, int dim[4], int shift[4], int post[4]);

static void rd_5dim(char **content, size_t *line_no, int *array, int dim[5], int shift[5]);

static void rd_5dim_slice(char **content, size_t *line_no, int *array, int dim[5], int shift[5], int post[5]);

static void rd_6dim(char **content, size_t *line_no, int *array, int dim[6], int shift[6]);

static void rd_6dim_slice(char **content, size_t *line_no, int *array, int dim[6], int shift[6], int post[6]);

static void rd_Tetraloop37(char **content, size_t *line_no);

static void rd_Triloop37(char **content, size_t *line_no);

static void rd_Hexaloop37(char **content, size_t *line_no);

static int set_parameters_from_string(char **file_content, const char *name);
    ...
    while ((line = file_content[line_no++])) {
        ...
        if (r == 1) {
            ...
            case IL_H:
                ...
                break;
            case MME:
                rd_3dim(file_content, &line_no, &(mismatchExt37[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MME_H:
                rd_3dim(file_content, &line_no, &(mismatchExtdH[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMH:
                rd_3dim(file_content, &line_no, &(mismatchH37[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMH_H:
                rd_3dim(file_content, &line_no, &(mismatchHdH[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMI:
                rd_3dim(file_content, &line_no, &(mismatchI37[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMI_H:
                rd_3dim(file_content, &line_no, &(mismatchIdH[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMI1N:
                rd_3dim(file_content, &line_no, &(mismatch1nI37[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMI1N_H:
                rd_3dim(file_content, &line_no, &(mismatch1nIdH[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMI23:
                rd_3dim(file_content, &line_no, &(mismatch23I37[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMI23_H:
                rd_3dim(file_content, &line_no, &(mismatch23IdH[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMM:
                rd_3dim(file_content, &line_no, &(mismatchM37[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case MMM_H:
                rd_3dim(file_content, &line_no, &(mismatchMdH[0][0][0]), mismatch_dim, mismatch_shift);
                break;
            case INT11:
                rd_4dim(file_content, &line_no, &(int11_37[0][0][0][0]), int11_dim, int11_shift);
                break;
            case INT11_H:
                rd_4dim(file_content, &line_no, &(int11_dH[0][0][0][0]), int11_dim, int11_shift);
                break;
            case INT21:
                rd_5dim(file_content, &line_no, &(int21_37[0][0][0][0][0]), int21_dim, int21_shift);
                break;
            case INT21_H:
                rd_5dim(file_content, &line_no, &(int21_dH[0][0][0][0][0]), int21_dim, int21_shift);
                break;
            case INT22:
                rd_6dim_slice(file_content, &line_no, &(int22_37[0][0][0][0][0][0]), int22_dim, int22_shift, int22_post);
                update_nst(int22_37);
                break;
            case INT22_H:
                rd_6dim_slice(file_content, &line_no, &(int22_dH[0][0][0][0][0][0]), int22_dim, int22_shift, int22_post);
                update_nst(int22_dH);
                break;
            case D5:
                rd_2dim(file_content, &line_no, &(dangle5_37[0][0]), dangle_dim, dangle_shift);
                break;
            case D5_H:
                rd_2dim(file_content, &line_no, &(dangle5_dH[0][0]), dangle_dim, dangle_shift);
                break;
            case D3:
                rd_2dim(file_content, &line_no, &(dangle3_37[0][0]), dangle_dim, dangle_shift);
                break;
            case D3_H:
                rd_2dim(file_content, &line_no, &(dangle3_dH[0][0]), dangle_dim, dangle_shift);
                break;
            case ML: {
                int values[6];
...
static void rd_1dim(char **content, size_t *line_no, int *array, int dim, int shift) { rd_1dim_slice(content, line_no, array, dim, shift, 0); }
...
static void rd_3dim(char **content, size_t *line_no, int *array, int dim[3], int shift[3]) {
    ...
    rd_3dim_slice(content, line_no, array, dim, shift, post);
}
...
static void rd_3dim_slice(char **content, size_t *line_no, int *array, int dim[3], int shift[3], int post[3]) {
    ...
    for (i = shift[0]; i < dim[0] - post[0]; i++) {
        rd_2dim_slice(content, line_no, array + (i * dim[1] * dim[2]), dim + 1, shift + 1, post + 1);
    }
    return;
}
...
static void rd_4dim(char **content, size_t *line_no, int *array, int dim[4], int shift[4]) {
    ...
    rd_4dim_slice(content, line_no, array, dim, shift, post);
}
...
static void rd_4dim_slice(char **content, size_t *line_no, int *array, int dim[4], int shift[4], int post[4]) {
    ...
    for (i = shift[0]; i < dim[0] - post[0]; i++) {
        rd_3dim_slice(content, line_no, array + (i * dim[1] * dim[2] * dim[3]), dim + 1, shift + 1, post + 1);
    }
    return;
}
...
static void rd_5dim(char **content, size_t *line_no, int *array, int dim[5], int shift[5]) {
    ...
    rd_5dim_slice(content, line_no, array, dim, shift, post);
}
...
static void rd_5dim_slice(char **content, size_t *line_no, int *array, int dim[5], int shift[5], int post[5]) {
    ...
    for (i = shift[0]; i < dim[0] - post[0]; i++)
        rd_4dim_slice(content, line_no, array + (i * dim[1] * dim[2] * dim[3] * dim[4]), dim + 1, shift + 1, post + 1);
    return;
}
...
static void rd_6dim_slice(char **content, size_t *line_no, int *array, int dim[6], int shift[6], int post[6]) {
    ...
    for (i = shift[0]; i < dim[0] - post[0]; i++)
        rd_5dim_slice(content, line_no, array + (i * dim[1] * dim[2] * dim[3] * dim[4] * dim[5]), dim + 1, shift + 1, post + 1);
    return;
}
...
static void check_symmetry(void) {
    ...
    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j <= NBPAIRS; j++)
            if (stack37[i][j] != stack37[j][i]) vrna_message_warning("stacking energies not symmetric");

    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j <= NBPAIRS; j++)
            if (stackdH[i][j] != stackdH[j][i]) vrna_message_warning("stacking enthalpies not symmetric");
            ...
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++)
                    if (int11_37[i][j][k][l] != int11_37[j][i][l][k])
                        vrna_message_warning("int11 energies not symmetric (%d,%d,%d,%d) (%d vs. %d)", i, j, k, l, int11_37[i][j][k][l],
                                             int11_37[j][i][l][k]);
    ...
    for (i = 0; i <= NBPAIRS; i++)
        for (j = 0; j <= NBPAIRS; j++)
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++)
                    if (int11_dH[i][j][k][l] != int11_dH[j][i][l][k]) vrna_message_warning("int11 enthalpies not symmetric");
        ...
        for (j = 0; j <= NBPAIRS; j++)
            for (k = 0; k < 5; k++)
                for (l = 0; l < 5; l++) {
                    ...
                    for (m = 0; m < 5; m++)
                        for (n = 0; n < 5; n++)
                            if (int22_37[i][j][k][l][m][n] != int22_37[j][i][m][n][k][l]) vrna_message_warning("int22 energies not symmetric");
                }
...
static void ignore_comment(char *line) {
    ...
    if ((cp1 = strstr(line, "/*"))) {
        cp2 = strstr(cp1, "*/");
        if (cp2 == NULL) vrna_message_error("unclosed comment in parameter file");
